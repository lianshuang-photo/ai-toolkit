    ...
    if (!gpuData) {
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
          <span className="block sm:inline">无 GPU 数据。(No GPU data available.)</span>
        </div>
      );
    }
    ...
    if (!gpuData.hasNvidiaSmi) {
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
          ...
          <span className="block sm:inline"> 此系统上 nvidia-smi 不可用。(nvidia-smi is not available on this system.)</span>
