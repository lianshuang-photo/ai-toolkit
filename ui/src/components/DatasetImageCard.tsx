      ...
      >
        ...
        {(!inViewport || !isVisible) && isCaptionLoaded && (
          <div className="w-full h-full flex items-center justify-center text-gray-400">
            {isVisible ? '滚动到视图以编辑标题 (Scroll into view to edit caption)' : '显示内容以编辑标题 (Show content to edit caption)'}
          </div>
        )}
        {!isCaptionLoaded && (
          <div className="w-full h-full flex items-center justify-center text-gray-400">加载标题中... (Loading caption...)</div>
        )}
      </div>
    </div>
  );
