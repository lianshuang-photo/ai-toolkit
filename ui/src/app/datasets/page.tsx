...
export default function Datasets() {
  ...
  const handleDeleteDataset = (datasetName: string) => {
    openConfirm({
      title: '删除数据集 (Delete Dataset)',
      message: `确定要删除数据集 "${datasetName}" 吗？此操作无法撤销。(Are you sure you want to delete the dataset "${datasetName}"? This action cannot be undone.)`,
      type: 'warning',
  ...
  const openNewDatasetModal = () => {
    openConfirm({
      title: '新建数据集 (New Dataset)',
      message: '输入新数据集的名称：(Enter the name of the new dataset:)',
      type: 'info',
      confirmText: '创建 (Create)',
      inputTitle: '数据集名称 (Dataset Name)',
  ...
  return (
    <>
      <TopBar>
        <div>
          <h1 className="text-2xl font-semibold text-gray-100">数据集 (Datasets)</h1>
        </div>
        ...
        <div className="space-y-4 text-gray-200">
          <form onSubmit={handleCreateDataset}>
            <div className="text-sm text-gray-400">
              这将在您的数据集文件夹中创建一个新文件夹。(This will create a new folder with the name below in your dataset folder.)
            </div>
            <div className="mt-4">
              <TextInput label="数据集名称 (Dataset Name)" value={newDatasetName} onChange={value => setNewDatasetName(value)} />
            </div>
