  ...
  return (
    <>
      ...
      <TopBar>
        ...
        <div>
          <h1 className="text-lg">任务 (Job): {job?.name}</h1>
        </div>
      ...
      <MainContent className={pages.find(page => page.value === pageKey)?.mainCss}>
        {status === 'loading' && job == null && <p>加载中... (Loading...)</p>}
        {status === 'error' && job == null && <p>获取任务出错 (Error fetching job)</p>}
        {job && (
          <>
            {pages.map(page => {
