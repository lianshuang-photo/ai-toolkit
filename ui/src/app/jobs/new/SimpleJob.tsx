      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className={topBarClass}>
          ...
          <Card title="保存 (Save)">
            ...
            />
            <NumberInput
              label="最多保留步数 (Max Step Saves to Keep)"
            ...
            <div className={trainingBarClass}>
              ...
              <div>
                ...
                {disableSections.includes('train.blank_prompt_preservation') ? null : (
                  <>
                    <Checkbox
                      label="空提示词保留 (Blank Prompt Preservation)"
        ...
        <div>
          <Card title="高级 (Advanced)" collapsible>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div>
                <Checkbox
                  label="使用差分引导 (Do Differential Guidance)"
                ...
                {jobConfig.config.process[0].train.differential_guidance_scale && (
                  <>
                    <NumberInput
                      label="差分引导比例 (Differential Guidance Scale)"
                      className="pt-2"
                ...
                <div key={i} className="p-4 rounded-lg bg-gray-800 relative">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div>
                      ...
                      />
                      <NumberInput
                        label="标题丢弃率 (Caption Dropout Rate)"
                        className="pt-2"
        ...
        <div>
          <Card title="采样 (Sample)">
            ...
            <FormGroup label={`采样提示词 (Sample Prompts) (${jobConfig.config.process[0].sample.samples.length})`} className="pt-2">
              <div></div>
            </FormGroup>
                        ...
                        <FormGroup label="控制图片 (Control Images)" className="pt-2 ml-4">
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mt-2 mt-2">
                            {['ctrl_img_1', 'ctrl_img_2', 'ctrl_img_3'].map((ctrlKey, ctrl_idx) => (
                              <SampleControlImage
                                key={ctrlKey}
                                instruction={`添加控制图片 ${ctrl_idx + 1} (Add Control Image ${ctrl_idx + 1})`}
                                className=""
        ...
        {status === 'success' && <p className="text-green-500 text-center">训练保存成功！(Training saved successfully!)</p>}
        {status === 'error' && <p className="text-red-500 text-center">保存训练时出错，请重试。(Error saving training. Please try again.)</p>}
      </form>
      <AddSingleImageModal />
    </>
  );
