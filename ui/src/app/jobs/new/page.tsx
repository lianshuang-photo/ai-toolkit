  ...
  return (
    <>
      <TopBar>
        ...
        <div>
          <h1 className="text-lg">{runId ? '编辑训练任务 (Edit Training Job)' : '新建训练任务 (New Training Job)'}</h1>
        </div>
        ...
        <div className="pr-2">
          ...
          >
            {showAdvancedView ? '显示简单模式 (Show Simple)' : '显示高级模式 (Show Advanced)'}
          </Button>
        </div>
        <div>
          <Button
          ...
          >
            {status === 'saving' ? '保存中... (Saving...)' : runId ? '更新任务 (Update Job)' : '创建任务 (Create Job)'}
          </Button>
        </div>
      </TopBar>
          ...
          <ErrorBoundary
            fallback={
              <div className="flex items-center justify-center h-64 text-lg text-red-600 font-medium bg-red-100 dark:bg-red-900/20 dark:text-red-400 border border-red-300 dark:border-red-700 rounded-lg">
                检测到高级任务。请切换到高级视图以继续。(Advanced job detected. Please switch to advanced view to continue.)
              </div>
            }
          >
            <SimpleJob
